        ...
        <DialogContent className="sm:max-w-2xl bg-card border-border">
          ...
          <Tabs defaultValue="iframe" className="w-full">
            ...
            <TabsContent value="iframe" className="space-y-4">
              <div className="bg-background/50 p-4 rounded-md border">
                <code className="text-sm break-all font-mono text-foreground">
                  {`<iframe src="${process.env.NEXTAUTH_URL || "https://tellus-omega.vercel.app"}/embed/${userCode}" width="100%" height="500" frameborder="0"></iframe>`}
                </code>
              </div>
              <Button
                onClick={() =>
            ...
            <TabsContent value="script" className="space-y-4">
              <div className="bg-background/50 p-4 rounded-md border">
                <code className="text-sm break-all font-mono text-foreground">
                  {`<script src="${process.env.NEXTAUTH_URL || "https://tellus-omega.vercel.app"}/embed.js" data-code="${userCode}"></script>`}
                </code>
              </div>
              <Button
                onClick={() =>
            ...
            <TabsContent value="api" className="space-y-4">
              <div className="bg-background/50 p-4 rounded-md border">
                <code className="text-sm break-all font-mono text-foreground">
                  {`GET ${process.env.NEXTAUTH_URL || "https://tellus-omega.vercel.app"}/api/testimonials/${userCode}`}
                </code>
              </div>
              <Button
                onClick={() =>
                  handleCopy(
                    `${process.env.NEXTAUTH_URL || "https://tellus-omega.vercel.app"}/api/testimonials/${userCode}`,
                    "api",
                  )
                }
                className="w-full"
